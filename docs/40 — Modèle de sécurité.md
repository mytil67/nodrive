...
| Expiration (filet de sécurité) | `Date.now() > expiresAt` | Cron à 3h UTC | Fallback |
| Chunks orphelins | Upload échoué sans metadata | Cron à 3h UTC | Nettoyage |

> La suppression après premier téléchargement est le comportement principal. L'expiration (24h par défaut) n'est qu'un filet de sécurité pour les transferts jamais récupérés.
